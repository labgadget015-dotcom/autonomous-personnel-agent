...
-- ============================================================
-- SELF-REFINING AGENT SYSTEM
-- ============================================================
-- Tables for the evaluation, reflection, prompt versioning, and
-- A/B testing pipeline that enables agents to improve over time.

-- Stores every evaluation result for every agent call
...
CREATE INDEX IF NOT EXISTS idx_outcomes_agent_ts ON agent_outcomes(agent, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_outcomes_passed ON agent_outcomes(agent, passed, created_at DESC);

-- Verbal reflections generated when score < threshold
...
CREATE INDEX IF NOT EXISTS idx_reflections_agent_active ON agent_reflections(agent, is_active);

-- Versioned prompts per agent — every change tracked with rollback support
...
CREATE INDEX IF NOT EXISTS idx_prompt_versions_agent_status ON prompt_versions(agent, status);

-- A/B evaluation run records
